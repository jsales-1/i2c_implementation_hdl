...
Revision Notes:
 ...
 - Separated asynchronous START/STOP events into dedicated always block for synthesis compatibility
 - Main FSM now uses single clock (SCL) with reset, meeting Genus synthesis requirements
 - Preserved immediate response to START/STOP conditions without multiple clock domains
*/
...
module i2c_slave_controller #(
    ...
    // All state transitions and data sampling occur on SCL rising edge
  	...
  	always @(posedge start_event or posedge stop_event) begin
    if (start_event) begin
    	state       <= READ_ADDR;
        counter     <= ADDR_WIDTH;
      	address_reg <= '0;
    end else if(stop_event && !start_event) 
            state <= IDLE;
