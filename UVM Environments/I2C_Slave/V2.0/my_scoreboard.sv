...
class my_scoreboard extends uvm_scoreboard;
    ...
    class i2c_slave_golden;
        ...
        function new(bit [6:0] addr = 7'h50);
            ...
            this.addr_ack_expected = 1;  // Always ACK own address
            this.data_ack_expected = 1;   // Always ACK data (for write)
            this.byte_count = 0;
        ...
        function void predict(my_seq_item item);
            ...
            if (match) begin
                if (item.is_write()) begin
                    ...
                    // No data ACK for reads (master sends ACK/NACK)
                end
            end
        endfunction
    ...
    function void build_phase(uvm_phase phase);
        ...
        `uvm_info(get_type_name(), $sformatf("Scoreboard built with slave_addr=0x%0h", 
                  slave_address), UVM_LOW)
    ...
    function void check_transaction(my_seq_item item);
        ...
        if (item.is_read() && item.addr_ack) begin
            bit [7:0] expected_data = golden_model.get_expected_read_data();
