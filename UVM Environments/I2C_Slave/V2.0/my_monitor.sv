...
class my_monitor extends uvm_monitor;
    ...
    task generate_simulated_transactions();
        ...
        // Gerar 10 transações simuladas
        for (int i = 0; i < 10; i++) begin
            simulated_item = my_seq_item::type_id::create("simulated_item");
            
            // Preencher com dados simulados
            simulated_item.target_addr = 7'h50;
            simulated_item.rw = (i % 2);  // Alterna entre read e write
            simulated_item.write_data = $urandom_range(0, 255);
            simulated_item.read_data = $urandom_range(0, 255);
            simulated_item.addr_ack = 1;  // Sempre ACK para addr 0x50
            simulated_item.data_ack = (simulated_item.rw == 0) ? 1 : 0;  // ACK para write
            simulated_item.start_detected = 1;
            simulated_item.stop_detected = 1;
            simulated_item.generate_stop = 1;
            simulated_item.bit_delay = 0;
            
            `uvm_info(get_type_name(), $sformatf("SIMULATED: Generated transaction %0d: %s", 
                      i, simulated_item.convert2string()), UVM_MEDIUM)
