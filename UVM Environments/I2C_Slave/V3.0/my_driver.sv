...
// Forward declaration para o compilador saber que my_monitor existe
...
class my_driver extends uvm_driver #(my_seq_item);
    ...
    task drive_transaction_simulated(my_seq_item item);
        ...
        static bit [7:0] last_written_data = 8'h00;
        my_monitor mon_handle;
        uvm_component comp;
        ...
        if (item.target_addr == 7'h50) begin
            ...
            // Simular data phase baseado no tipo de operação
            if (item.is_write()) begin
                // Para write: ACK e armazenar dados
                item.data_ack = 1;
                last_written_data = item.write_data;
                write_count++;
                `uvm_info(get_type_name(), $sformatf("SIMULATED: Write data 0x%0h ACKed (write #%0d)", 
                          item.write_data, write_count), UVM_MEDIUM)
            end
            else begin
                // Para read: retornar o último dado escrito ou 0xA5
                if (write_count > 0) begin
                    item.read_data = last_written_data;
                    `uvm_info(get_type_name(), $sformatf("SIMULATED: Read data 0x%0h (last written)", 
                              item.read_data), UVM_MEDIUM)
                end else begin
                    item.read_data = 8'hA5;  // Default value
                    `uvm_info(get_type_name(), $sformatf("SIMULATED: Read data 0x%0h (default)", 
                              item.read_data), UVM_MEDIUM)
                end
            end
        end
        ...
        if (item.generate_stop) begin
            #10;
            `uvm_info(get_type_name(), "SIMULATED: STOP generated", UVM_HIGH)
        end
        
        // CORREÇÃO: Encontrar o monitor no mesmo nível do agent
        comp = this.get_parent().get_child("mon");
        if (comp != null) begin
            $cast(mon_handle, comp);
            mon_handle.add_driver_item(item);
            `uvm_info(get_type_name(), $sformatf("Item sent to monitor queue. Queue size: %0d", 
                      mon_handle.get_queue_size()), UVM_HIGH)
        end else begin
            `uvm_error(get_type_name(), "Could not find monitor handle")
        end
